...
Во многих случаях f-строки удобней использовать, так как шаблон выглядит понятней и писать меньше. Однако, бывают случаи, когда метод format удобней. Например:
```python
In [6]: ip = [10, 1, 1, 1]
...
F-строка выполнится первый раз и подставит текущие значения переменных и для использования шаблона еще раз, его надо аново писать. Это значит, что будут копии одной и то же строки по скрипту.
